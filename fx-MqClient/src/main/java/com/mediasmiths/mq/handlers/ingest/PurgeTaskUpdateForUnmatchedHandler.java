...
public class PurgeTaskUpdateForUnmatchedHandler extends TaskUpdateHandler
{

	private final static Logger log = Logger.getLogger(PurgeTaskUpdateForUnmatchedHandler.class);
	...
	public void onTaskUpdate(AttributeMap currentAttributes, AttributeMap before, AttributeMap after)
	{
		...
		{
			log.debug("asset is not of type Material.");
		...
		{
			log.info(String.format("Asset %s (%s) is now ready for qc", houseID, assetID));

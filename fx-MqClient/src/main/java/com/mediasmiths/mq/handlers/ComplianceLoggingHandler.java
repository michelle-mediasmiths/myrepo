...
public class ComplianceLoggingHandler  implements AttributeHandler
{
	MayamTaskController taskController;
	private final static Logger log = Logger.getLogger(ComplianceLoggingHandler.class);
	
	public ComplianceLoggingHandler(MayamTaskController controller) 
	{
		...
		if (taskListID.equals(MayamTaskListType.COMPLIANCE_LOGGING.getText())) 
		{
			try {
				...
				if (taskState == TaskState.FINISHED) 
				{
					...
					long taskID = taskController.createTask(assetID, MayamAssetType.fromString(assetType.toString()), MayamTaskListType.COMPLIANCE_EDIT);

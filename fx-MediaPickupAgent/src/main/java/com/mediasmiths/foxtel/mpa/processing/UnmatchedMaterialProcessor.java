...
public class UnmatchedMaterialProcessor implements Runnable
{
	...
	{
		Collection<MaterialEnvelope> unmatchedMessages = matchMaker.purgeUnmatchedMessages(timeout.longValue());

		if (unmatchedMessages.size() > 0)
		{
			logger.info(String.format("Found %d unmatched material messages", unmatchedMessages.size()));
		}
		else
		{
			logger.debug(String.format("Found %d unmatched material messages", unmatchedMessages.size()));
		}

		for (MaterialEnvelope me : unmatchedMessages)
		{
			logger.info(String.format("no mxf for %s", me.getFile().getAbsolutePath()));
			...
			sb.append(String.format(
					...
					FilenameUtils.getName(me.getFile().getAbsolutePath()),
					me.getMasterID()));

		...
		// Title ID of temporary material updated - add to source ids of title, remove material from any purge lists
			...
			try {			
				if (assetType.equals(MayamAssetType.MATERIAL.getAssetType()) && attributeChanged(Attribute.ASSET_PEER_ID, before, after,currentAttributes)) 
				{
					...
					taskController.createQCTaskForMaterial(matchedAsset.getAttributeAsString(Attribute.HOUSE_ID), (Date) matchedAsset.getAttribute(Attribute.COMPLETE_BY_DATE),matchedAsset.getAttributeAsString(Attribute.QC_PREVIEW_RESULT), matchedAsset);
				}
			}
			catch (Exception e) {

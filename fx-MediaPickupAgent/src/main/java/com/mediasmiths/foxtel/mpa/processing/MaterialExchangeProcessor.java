...
public class MaterialExchangeProcessor extends MediaPickupProcessor<Material>
{
	 ...
	 */
	@Override
	protected MamUpdateResult updateMamWithMaterialInformation(Material message) throws MessageProcessingFailedException
	{
		...
		if (Util.isProgramme(message))
		{
			...
			boolean waitForMedia = updateProgrammeMaterial(
					message.getTitle().getProgrammeMaterial(),
					message.getDetails(),
					message.getTitle());
		...
		else
		{
			...
			try
			{
				...
				String masterID = mayamClient.createMaterial(
						message.getTitle().getTitleID(),
						message.getTitle().getMarketingMaterial(),
						message.getDetails(),
						message.getTitle());
			...
			{
				logger.error("MayamClientException update mam with marketing material information", e);
				throw new MessageProcessingFailedException(MessageProcessingFailureReason.MAYAM_CLIENT_EXCEPTION, e);
			}
			catch (IllegalArgumentException iae)
			{
				logger.error("Illegal argument exception in mayam client", iae);
				throw new MessageProcessingFailedException(MessageProcessingFailureReason.MAYAM_CLIENT_ARGUMENT_EXCEPTION, iae);
			}
	...
	/**
	 * Creates or updates a title in viz ardome, should be used for marketing material only as placeholders for programmes should exist
	 * 
	 * @param title
	 ...
	 */
	private boolean createOrUpdateTitle(Title title) throws MayamClientException, MessageProcessingFailedException
	{
		...
		{
			logger.error(String.format("Error creating or updating title %s", title.getTitleID()));
	...
	/**
	 * Update any missing metadata for the title in viz ardome with the aggregator information from the xml file
	 * 
	 * @param title
	 * @throws MessageProcessingFailedException
	 */
	...
	/**
	 * Update any missing metadata for the material\item in viz ardome with the aggregator information
	 * 
	 ...
	 * @throws MessageProcessingFailedException
	 */
	private boolean updateProgrammeMaterial(
			ProgrammeMaterialType programmeMaterial,
			Material.Details details,
			Material.Title title) throws MessageProcessingFailedException
	{
